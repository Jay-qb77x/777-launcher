  ...
  return {
    ...
    scripts: list(m.scripts, 100)
      ...
      .map((s) => ({
        ...
        // `url` is kept for older entries. `videoUrl` / `tebexUrl` power the
        // Buy on Tebex / Watch video buttons in the details popup.
        url: isHttps(str(s.url, 500)) ? str(s.url, 500) : '',
        videoUrl: isHttps(str(s.videoUrl, 500)) ? str(s.videoUrl, 500) : '',
        tebexUrl: isHttps(str(s.tebexUrl, 500)) ? str(s.tebexUrl, 500) : '',
      }))
      // Needs a title and at least one working link (old `url`, or the new fields).
      .filter((s) => s.title && (s.url || s.videoUrl || s.tebexUrl)),
